...
char* esp8266_InfCon(void);

unsigned char *esp8266_Client_Request(unsigned char* host, unsigned char* url);

unsigned char esp8266_config_Station(unsigned char* softssid, unsigned char* softpass);

unsigned char *esp8266_current_time();
...
inline unsigned char _esp8266_waitResponse(void) {
    ...
    unsigned const char* strings[6] = {"OK", "ready", "FAIL", "no change", "Linked", "Unlink"}; //respostas
...
void envia_pagina()
{      
    _esp8266_print("AT+CIPSEND=0,381\r\n");  //usar: https://www.invertexto.com/contador-caracteres
    __delay_ms(300);                                               
    _esp8266_print("<html><head><title>PSQ</title><meta http-equiv=\"refresh\" content=\"4\" ></head><h1>WEB LED!</h1>"); //94
    _esp8266_print("<body><p><form method=\"GET\"></p>"); //32                             
    _esp8266_print("<p><input type=\"submit\" name=\"B\" value=\"1\" > LED1 ON</p>");       //56
    _esp8266_print("<p><input type=\"submit\" name=\"B\" value=\"2\" > LED1 OFF</p>");    //57
    _esp8266_print("<p><input type=\"submit\" name=\"B\" value=\"3\" > LED2 ON</p>");       //56
    _esp8266_print("<p><input type=\"submit\" name=\"B\" value=\"4\" > LED2 OFF</p>");   //57

    if (PORTAbits.RA3){
        _esp8266_print( "<p>RA3 ON! </p></body></html>\r\n");  //29
    }
    else _esp8266_print("<p>RA3 OFF!</p></body></html>\r\n");  //29
...
inline unsigned char _esp8266_waitResponse2(void) {
    unsigned char so_far[4] = {0,0,0,0};        //quantidade de respostas
    unsigned const char lengths[4] = {3,3,3,3}; //tamanho das respostas
    unsigned const char* strings[4] = {"B=4", "B=3", "B=2", "B=1"}; //respostas
    unsigned const char responses[4] = {LED2OFF, LED2ON, LED1OFF, LED1ON};
    unsigned char received;
    unsigned char response;
    bool continue_loop = true;
    while (continue_loop) {
 ...
 */     
    ...
    //Lcd_Set_Cursor(2,1);
    //Lcd_Write_String("MUX ativado");
    //__delay_ms(1000);
    
    _esp8266_create_server();  
    
    //Lcd_Set_Cursor(2,1);
    //Lcd_Write_String("Web OK");
    //__delay_ms(1000);
    
    //_esp8266_print("AT+CWDHCP=0,0\r\n")   //Desabilita DHCP - MODO STA
    //_esp8266_print("AT+CIPAP=\"192.168.5.5\"\r\n");   //Escolher ip MODO STA
...
{    
    ...
    _esp8266_print("GET ");
    _esp8266_print(url);
    _esp8266_print(" HTTP/1.1\r\n");
    _esp8266_print("Host: ");
    _esp8266_print(host);
    _esp8266_print("\r\n");
    _esp8266_print("Connection: close\r\n\r\n"); 
    */
    _esp8266_print("AT+HTTPCLIENT=\"");
    _esp8266_print("2""\",\"");
    _esp8266_print("3""\",\"");
    _esp8266_print(host);
    _esp8266_print("\",\"");
...
unsigned const char* esp8266_getAPI()
{
    ...
    _esp8266_waitFor("OK");
    _esp8266_print("AT+CIPSTART=\"TCP\",\"www.kma.go.kr\",80\r\n");  
    _esp8266_waitFor("OK");
